...
class STMAML(nn.Module):
    ...
    def finetuning(self, target_dataloader, target_epochs):
        """
        finetunning stage in MAML
        """
        self.M_maml_model = deepcopy(self.model)

        optimizer = optim.Adam(self.M_maml_model.parameters(), lr=self.meta_lr, weight_decay=1e-2)
        min_MAE = 10000000
        best_result = ''
        best_meta_graph = -1
        ...
        for epoch in tqdm(range(target_epochs)):
            ...
            for step, (data, A_wave) in enumerate(target_dataloader):
                ...
                if self.model_name == 'GWN':
                    ...
                    out, meta_graph = self.M_maml_model(data, adj_mx)
                else:
                    out, meta_graph = self.M_maml_model(data, A_wave[0].float())
    ...
    def evaluate(self, test_dataloader): 
        ...
        with torch.no_grad():
            ...
            for step, (data, A_wave) in enumerate(test_dataloader):
                ...
                if step == 0:
                    outputs = out
                    print('step 0 outputs shape =', outputs.shape)
                    y_label = data.y
                else:
                    outputs = torch.cat((outputs, out))
                    print('step %d outputs shape ='%step, outputs.shape)
